...
//定义全局变量
int my_work_mode = 3;//交换机:2;路由器：3
int serial_num = 0;//发
...
int main(int argc, char* argv[])
{
	...
	//路由表初始化
	rt[0].destination = 1; rt[0].gataway = 2; rt[0].port = 0; rt[0].metric = 2;
	rt[1].destination = 2; rt[1].gataway = 2; rt[1].port = 0; rt[1].metric = 1;
	rt[2].destination = 4; rt[2].gataway = 4; rt[2].port = 1; rt[2].metric = 2;
	rt[3].destination = 5; rt[3].gataway = 2; rt[3].port = 0; rt[3].metric = 3;
	rt[4].destination = 6; rt[4].gataway = 2; rt[4].port = 0; rt[4].metric = 3;
		...
		if (FD_ISSET(sock, &readfds)) {
			...
			if (remote_addr.sin_port == upper_addr.sin_port) {
				...
				if (1) {
					...
					if (send_port == 0)Sleep(10);
					if (send_port == 1)Sleep(20);
					if (send_port == -1)
					{
						Sleep(10);
